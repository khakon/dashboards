var SharedWidgets = function () {
    ...
    self.initRange = function (selectedRangeChanged, customOptions) {
        ...
        var options = {
            ...
            dataSource: [],
            selectedRange: salesRangeSelectedRange,
            onSelectedRangeChanged: selectedRangeChanged
        };
        ...
        var drawRangeSelector = function(dataSource, salesRangeSelectedRange) {
            range.option("dataSource", dataSource);
            range.option("selectedRange", salesRangeSelectedRange);
        },

        rangeYear = getCurrentDate().getFullYear(),
        nextYear = $("#nextYear"),
        prevYear = $("#prevYear"),
        changeRangeYear = function(offset) {
            ...
            function setRangeYear(year) {
                ...
                if (year <= new Date().getFullYear() - 4) {
    ...
    //        window.open("https://js.devexpress.com/", "_blank");
    //    });
    ...
    self.getMaxAxisValue = function (data, dataField) {
        var maxsSeries = [4000, 8000, 12000, 20000, 40000, 80000, 120000, 200000, 400000, 800000, 1200000, 2000000, 4000000, 8000000, 12000000, 20000000, 40000000, 80000000, 120000000, 200000000],
            max = 0,
            index = 0;
        ...
        $.each(maxsSeries, function (i, value) {
            if (value >= max) {
                index = i;
                return false;
            }
        });
        return maxsSeries[index];
    }
